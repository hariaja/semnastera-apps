...
@section('content')
  <div class="block block-rounded">
    ...
    <div class="block-content block-content-full">
      ...
      <form action="{{ route('registrations.update', $registration->code) }}" method="POST">
        @csrf
        @method('patch')
        ...
        <div class="row">
          <div class="col-md">
            <div class="mb-3">
              ...
              <input type="text" class="form-control @error('title') is-invalid @enderror" name="title" id="title" value="{{ old('title', $registration->title) }}" placeholder="{{ trans('Judul') }}">
              @error('title')
          ...
          <div class="col-md">
            <div class="mb-3">
              ...
              <input type="date" class="form-control @error('date_start') is-invalid @enderror" name="date_start" id="date_start" value="{{ old('date_start', date('Y-m-d', strtotime($registration->date_start->toDateString()))) }}" placeholder="{{ trans('Y-m-d') }}">
              @error('date_start')
                <div class="invalid-feedback"><b>{{ $message }}</b></div>
              @enderror
            </div>
        ...
        <div class="row">
          <div class="col-md">
            <div class="mb-3">
              ...
              <input type="text" class="form-control @error('date_end') is-invalid @enderror" name="date_end" id="date_end" value="{{ old('date_end', date('Y-m-d', strtotime($registration->date_end->toDateString()))) }}" placeholder="{{ trans('Y-m-d') }}" readonly>
              @error('date_end')
                <div class="invalid-feedback"><b>{{ $message }}</b></div>
              @enderror
            </div>
          ...
          <div class="col-md">
            <div class="mb-3">
              ...
              <select name="status" id="status" class="form-select @error('status') is-invalid @enderror">
                ...
                <option value="1" {{ old('status', $registration->status) == '1' ? 'selected' : '' }}>{{ trans('Active') }}</option>
                <option value="0" {{ old('status', $registration->status) == '0' ? 'selected' : '' }}>{{ trans('Inactive') }}</option>
              </select>
              @error('status')
                <div class="invalid-feedback"><b>{{ $message }}</b></div>
              @enderror
